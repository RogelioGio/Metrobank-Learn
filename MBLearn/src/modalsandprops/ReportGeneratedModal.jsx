...
const ReportGeneratedModal = ({open,close,type,message,classname}) => {
    return (
                            ...
                            {/* Header */}
                                <div className="py-4 mx-4 border-divider flex flex-col items-center justify-center gap-2">
                                    <div className="w-24 aspect-square bg-secondaryprimary rounded-full text-center flex items-center justify-center text-primary">
                                        <FontAwesomeIcon icon={type === true ? faCircleCheck : faCircleXmark} className="text-primary text-4xl"/>
                                    </div>
                                    <div className="text-center flex-col flex gap-1">
                                        {type}
                                        <h1 className="text-primary font-header text-3xl">{type === true ? "Report Generated" : "Generation Failed"}</h1>
                                        <p className="text-unactive font-text text-sm">{type === true ? "The requested report is successfully generated" : "Report Generation Failed"}</p>
                                        {
                                            type !== true &&
                                            <p className="text-unactive font-text text-sm">{message}</p>
                                        }
                                    </div>
                                </div>
