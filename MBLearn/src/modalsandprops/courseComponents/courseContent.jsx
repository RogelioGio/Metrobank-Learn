...
const Content = ({ course }) => {
    ...
    useEffect(() => {
        if (course) {
            ...
            const courseData = JSON.parse(course); // Assuming course is already a parsed object
        ...
        <>
            {(() => {
                ...
                lesson?.content?.forEach((content, index) => {
                    ...
                    if (content.type === "codeBlock") {
                        ...
                        renderedContent.push(
                            <pre
                            key={`codeblock-${index}`}
                            className="bg-gray-900 p-3 rounded overflow-auto my-4"
                            style={{ color: '#ccc', whiteSpace: 'pre-wrap', wordBreak: 'break-word' }} // light gray text
                            >
                            <code className="bg-gray-700 p-2 rounded block font-mono text-gray-300 text-sm">
                                {codeText}
                            </code>
                            </pre>
                        );
                    ...
                    if (content.type === "horizontalRule") {
                        renderedContent.push(
                            <hr key={index} className="border-black border my-4 rounded-md" />
                        );
                    }
                });
                ...
                return (
                    ...
                    // <ScrollArea className="h-[calc(100vh-11rem)] w-full px-2 rounded-md bg-white">
