        ...
        <div class="content">
            ...
            <div style="margin-top: 10px; margin-bottom:10px ;">
                <div>
                    ...
                    <h1 style="font-size: 70px; margin: 0px;">{{ $name }}</h1>
                    <p style="margin: 0px;">from the course</p>
                    <h1 style="font-size: 30px; margin: 0px;">{{ $course }}</h1>
                </div>
                <div>
                    ...
                    <h1 style="font-size: 20px; line-height: 80%;">{{ $date_of_completion }}</h1>
                </div>
            </div>
            <div style="margin-top: 30px;">
                ...
                <div class="signatures">
                    @forelse($creditors as $creditor)
                        <div>
                            @if($creditor->creditor_signature_img_url)
                                <img src="{{ $creditor->creditor_signature_img_url}}"
                                    alt="{{ $creditor->creditor_signature_img_url}}"
                                    style="height: 50px; margin-bottom: 5px;" />
                            @else
                                <p style="margin: 0px;">_____________________________</p>
                            @endif
                            <h1 style="font-size: small; margin: 0px;">
                                {{ $creditor->CreditorName ??  $creditor->creditor_name ??'Personnel Name' }}
                            </h1>
                            <p style="font-size: smaller; margin: 0px;">
                                {{ $creditor->CreditorPosition?? $creditor->creditor_position ?? 'Title and Position' }}
                            </p>
                        </div>
                    @empty
                        @for($i = 0; $i < 3; $i++)
